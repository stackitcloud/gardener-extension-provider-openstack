...
func NewValuesProvider(logger logr.Logger, useTokenRequestor, useProjectedTokenMount bool, csi *config.CSI) genericactuator.ValuesProvider {
...
func (vp *valuesProvider) GetControlPlaneChartValues(
	...
	return getControlPlaneChartValues(cpConfig, cp, cluster, userAgentHeaders, checksums, scaledDown, vp.useTokenRequestor, vp.csi)
}
...
func getCSIControllerChartValues(cluster *extensionscontroller.Cluster, userAgentHeaders []string, checksums map[string]string, scaledDown bool, csi *config.CSI) (map[string]interface{}, error) {
	k8sVersionLessThan119, err := version.CompareVersions(cluster.Shoot.Spec.Kubernetes.Version, "<", openstack.CSIMigrationKubernetesVersion)
	if err != nil {
		return nil, err
	}
