...
const (
	...
	// TerraformOutputKeyRouterIDv6 is the id the v6 router between provider network and the worker subnet.
	...
	// TerraformOutputKeySubnetIDv6 is the id of the v6 worker subnet.
	TerraformOutputKeySubnetIDv6 = "subnet_id_v6"
...
func ComputeTerraformerTemplateValues(
	...
	for _, val := range strings.Split(workersCIDR, ",") {
	...
	externalNetworkID := ""
	if config.Networks.ExternalNetworkID != nil {
		externalNetworkID = *config.Networks.ExternalNetworkID
	}

	subnetPoolID := ""
	if config.Networks.SubnetPoolID != nil {
...
type TerraformState struct {
	...
	RouterIP string
	// RouterIDv6 is the id the v6 router between provider network and the worker subnet.
	RouterIDv6 string
	// NetworkID is the private worker network.
	NetworkID string
	// NetworkIDv6 is the private v6 worker network.
	NetworkIDv6 string
	// NetworkName is the private worker network name.
	NetworkName string
	// SubnetID is the id of the worker subnet.
	SubnetID string
	// SubnetIDv6 is the id of the v6 worker subnet.
	SubnetIDv6 string
